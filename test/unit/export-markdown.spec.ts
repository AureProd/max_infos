...
const { articleToMarkdown } = await import('../../server/utils/export')
...
describe('articleToMarkdown', () => {
  it('carries every field needed to rebuild the article', () => {
    const md = articleToMarkdown(ARTICLE, ['mémoire', 'Algérie'])
  ...
  it('escapes the quotes that would break the front matter', () => {
    ...
    const md = articleToMarkdown({ ...ARTICLE, title: 'Le "grand" soir' }, [])
  ...
  it('escapes the backslash before the quote, not after', () => {
    ...
    expect(articleToMarkdown({ ...ARTICLE, title: 'chemin\\' }, [])).toContain(
      'title: "chemin\\\\"',
    )
    expect(articleToMarkdown({ ...ARTICLE, title: 'a\\"b' }, [])).toContain('title: "a\\\\\\"b"')
  })
  ...
  it('writes the publication date in three shapes without losing it', () => {
    ...
    expect(articleToMarkdown(ARTICLE, [])).toContain('publishedAt: "2026-03-04T08:00:00.000Z"')
    expect(
      articleToMarkdown({ ...ARTICLE, publishedAt: '2026-03-04T08:00:00.000Z' }, []),
    ).toContain('publishedAt: "2026-03-04T08:00:00.000Z"')
    expect(articleToMarkdown({ ...ARTICLE, publishedAt: null }, [])).toContain('publishedAt: ""')
  })

  it('lists the tags as a YAML array', () => {
    expect(articleToMarkdown(ARTICLE, [])).toContain('tags: []')
    expect(articleToMarkdown(ARTICLE, ['a', 'b'])).toContain('tags: ["a", "b"]')
    expect(articleToMarkdown(ARTICLE, ['l\'"IA"'])).toContain('tags: ["l\'\\"IA\\""]')
  })
  ...
  it('turns a missing field into an empty string, never into "undefined"', () => {
    ...
    const md = articleToMarkdown({ slug: 'x' }, [])
