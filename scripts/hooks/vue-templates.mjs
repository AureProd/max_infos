 ...
 * flags every <DataTable> and blocks the commit. Read from the installed
 * package rather than written down: a list copied by hand goes stale at the
 * next update.
 *
 * Compared in lower case: the folder is `datatable`, the component
 * <DataTable>.
 */
function primevueComponents(root) {
  try {
    ...
    // PrimeVue absent: the guard simply has nothing extra to allow.
    return new Set()
  }
}
