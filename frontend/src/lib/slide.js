...
function block(lines, { x, y, size, lead, weight, fill, family }) {
  return lines
    .map(
      (l, i) =>
...
export function slide(spec, handle = '@unmaxdinfo_') {
  const S = 1080
  const P = 88
  ...
  return {
    viewBox: `0 0 ${S} ${S}`,
    inner: parts.join(''),
    label: spec.alt || spec.title || spec.label || spec.text || 'Diapositive'
  }
}
